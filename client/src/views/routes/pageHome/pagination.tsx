...
export function calculatePaginationItems(pageNumber: number) {
  return { start: CardsPerPage * (pageNumber - 1) + 1, end: CardsPerPage * pageNumber };
}

export function paginationData(props: { filteredData: InterfaceData[]; currentPageNumber: number }) {
  let { start, end } = calculatePaginationItems(props.currentPageNumber);
  return props.filteredData.slice(start - 1, end);
}

export function paginationText(props: { currentPageNumber: number; totalItems: number }) {
  let { start, end } = calculatePaginationItems(props.currentPageNumber);
...
export const AppPagination = (props: Props) => (
  <Container textAlign="center">
    <Pagination
      ...
      onPageChange={(_, { activePage }) => props.handleOnPageChange(activePage as number)}
    />
  </Container>
);
