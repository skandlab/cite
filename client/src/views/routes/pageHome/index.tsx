...
const requestMetadata = () => axios.request({ method: "GET", url: API_METADATA() });
...
export class Home extends React.Component<{}, State> {
  ...
  updateLigandOrReceptor = (_ligandOptions: ColumnBrowserProps[], _receptorOptions: ColumnBrowserProps[]) => {
        ...
        <Grid.Row centered>
          <GridColumn>
            <ColumnBrowserGroup
              {...this.state}
              {...this}
              updateTumorOptions={(value) => this.setState({ tumorOptions: value })}
              updatePairsOptions={(value) => this.setState({ pairsOptions: value })}
            />
          </GridColumn>
        </Grid.Row>

        {this.state.filteredData.length !== 0 && !this.state.isFetchingData && (
          <>
            <Grid.Row centered>
              <GridColumn>
            ...
            <Grid.Row centered>
              <GridColumn>
                <HeatMapCards data={paginationData({ ...this.state })} {...this.state} />
              </GridColumn>
            </Grid.Row>
            <Grid.Row centered>
              <GridColumn>
                <AppPagination
                  {...this.state}
                  handleOnPageChange={(activePageNumber) => this.setState({ currentPageNumber: activePageNumber })}
                />
              </GridColumn>
            </Grid.Row>
          </>
