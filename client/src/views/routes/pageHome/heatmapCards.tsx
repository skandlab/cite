...
export const HeatMapCards = (props: Props) => {
  let _pairKeys = props.pairsOptions.filter((pair) => pair.isChecked).map((pair) => pair.value);
  let _tumorKeys = props.tumorOptions.filter((tumor) => tumor.isChecked).map((tumor) => tumor.value);
            ...
            <Card.Header>
              <Statistic.Group widths={2}>
                <Statistic className="extra" size="mini" text>
                  <Statistic.Value>{_data.ligand}</Statistic.Value>
                  <Statistic.Label>Ligand</Statistic.Label>
                </Statistic>
                <Statistic className="extra" size="mini" text>
                  <Statistic.Value>{_data.receptor}</Statistic.Value>
                  <Statistic.Label>Receptor</Statistic.Label>
                </Statistic>
              </Statistic.Group>
            </Card.Header>
            <Card.Description style={{ height: "320px" }}>
              <HeatMap data={_data} pairKeys={_pairKeys} tumorKeys={_tumorKeys} />
            </Card.Description>
          </Card.Content>
        </Card>
      ))}
